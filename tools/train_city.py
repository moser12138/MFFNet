...
from eval_city import eval_model
...
def set_meters():
    ...
    loss_main_meter = AvgMeter('loss_main')
    loss_aux_meter = AvgMeter('loss_pre')

    return time_meter, loss_meter, loss_main_meter, loss_aux_meter
...
def train():
    ...
    ## meters
    time_meter, loss_meter, loss_main_meter, loss_aux_meter = set_meters()
    ...
    for it, (im, lb) in enumerate(dl):
        ...
        with amp.autocast(enabled=cfg.use_fp16):
            logits, logit_aux = net(im)
            loss_main = criteria_pre(logits, lb)
            loss_aux = criteria_pre(logit_aux, lb)
        ...
        loss_main_meter.update(loss_main.item())
        loss_aux_meter.update(loss_aux.item())
        ...
        if (it + 1) % 100 == 0:
            ...
            lr = sum(lr) / len(lr)
            print_log_msg(
                it, cfg.max_iter, lr, time_meter, loss_meter, loss_main_meter,
                loss_aux_meter)
        lr_schdr.step()
